...
test("guest fallback starts after first failed state without waiting for second failed event", async ({
    page,
}) => {
    await openHome(page);
                ...
                await new Promise((resolve, reject) => {
                    ...
                    const timer = setInterval(() => {
                        ...
                        if (Date.now() - started > 8_000) {
                            clearInterval(timer);
                            reject(
                                new Error("Guest fallback did not trigger relay setup in time."),
                            );
                        }
                    }, 25);
                });
                ...
                await new Promise((resolve, reject) => {
                    ...
                    const timer = setInterval(() => {
                        ...
                        if (Date.now() - started > 5000) {
                            ...
                            reject(
                                new Error(
                                    "MQTT mock did not emit CONNECT and SUBSCRIBE packets in time.",
                                ),
                            );
                        }
                    }, 20);
...
test("mqtt guest inbound stall triggers recovery reconnect", async ({ page }) => {
    ...
    const result = await page.evaluate(
        async ({ fnSources }) => {
            ...
            try {
                const { state } = window.__planningPokerE2E;
                const { onHostChannelClose, onHostChannelOpen, runGuestMqttHealthCheckForTest } =
                ...
                await new Promise((resolve, reject) => {
                    ...
                    const timer = setInterval(() => {
                        if (
                            (window.__PP_MQTT_CONNECT_COUNT || 0) >= 1 &&
                            /reconnecting|unstable/i.test(state.guestConnectionPhase)
                        ) {
                            clearInterval(timer);
                            resolve();
                            return;
...
test("host broadcast targets each guest explicitly", async ({ page }) => {
    ...
    expect(result.some((msg) => msg.t === "state" && msg.to === "guest-a" && msg.round === 3)).toBe(
        true,
    );
    expect(result.some((msg) => msg.t === "state" && msg.to === "guest-b" && msg.round === 3)).toBe(
        true,
    );
});
